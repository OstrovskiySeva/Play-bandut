...
class Play:
    ...
    def initialize_screen(self):
        ...
        icon = pygame.image.load("assets/image/bandit.png")
        pygame.display.set_icon(icon)

    def initialize_images(self):
        self.wall_img = pygame.image.load("assets/image/wall.png").convert_alpha()
        self.wall_img = pygame.transform.smoothscale(self.wall_img, (TILE_SIZE, TILE_SIZE))
        self.floor_img = pygame.image.load("assets/image/floor.png").convert_alpha()
        self.floor_img = pygame.transform.smoothscale(self.floor_img, (TILE_SIZE, TILE_SIZE))
        self.bomba_img = pygame.image.load("assets/image/gold.png").convert_alpha()
        self.bomba_img = pygame.transform.smoothscale(self.bomba_img, (TILE_SIZE, TILE_SIZE))
        self.player_up_img = pygame.image.load("assets/image/player_up.png").convert_alpha()
        self.player_up_img = pygame.transform.smoothscale(self.player_up_img, (TILE_SIZE, TILE_SIZE))
        self.player_down_img = pygame.image.load("assets/image/player_down.png").convert_alpha()
        self.player_down_img = pygame.transform.smoothscale(self.player_down_img, (TILE_SIZE, TILE_SIZE))
        self.player_left_img = pygame.image.load("assets/image/player_left.png").convert_alpha()
        self.player_left_img = pygame.transform.smoothscale(self.player_left_img, (TILE_SIZE, TILE_SIZE))
        self.player_right_img = pygame.image.load("assets/image/player_right.png").convert_alpha()
        self.player_right_img = pygame.transform.smoothscale(self.player_right_img, (TILE_SIZE, TILE_SIZE))
        self.x_img = pygame.image.load("assets/image/X.png").convert_alpha()
        ...
        self.bullet_right_img = pygame.image.load("assets/image/bullet.png").convert_alpha()
        ...
        self.box_img = pygame.image.load("assets/image/box.png").convert_alpha()
        self.box_img = pygame.transform.smoothscale(self.box_img, (TILE_SIZE, TILE_SIZE))
    ...
    def initialize_sound(self):
        ...
        self.bomba_sound = pygame.mixer.Sound("assets/sound/bomba.mp3")
        self.bomba_sound.set_volume(0.7)
        self.shoot_sound = pygame.mixer.Sound("assets/sound/shoot.mp3")
        self.shoot_sound.set_volume(0.5)
        self.pol_shoot_sound = pygame.mixer.Sound("assets/sound/shoot_pol_sound.mp3")
        self.pol_shoot_sound.set_volume(0.5)
        self.sounds = ['assets/sound/fon.mp3', 'assets/sound/fon2.mp3', 'assets/sound/fon1.mp3']
            ...
            for line in file:
                ...
                if line:
                    if line.startswith("LEVEL"):
                        level = {"map": [], "player": [], "bomba": [], "polica": [], "dop_polica": [], "box": []}
    ...
    def rounded_cone(self):
        self.maska_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        self.maska_surface.fill((0, 0, 0, 0))
        ...
        if self.dir == "up":
            ...
            self.direction = math.radians(180)
        elif self.dir == "right":
            self.direction = math.radians(0)

        x, y = self.level_copy["player"][0]
        center_x = x * TILE_SIZE + TILE_SIZE // 2 + self.start_x
        center_y = y * TILE_SIZE + TILE_SIZE // 2 + self.start_y

        start_angle = self.direction - (math.pi / 2) / 2
        end_angle = self.direction + (math.pi / 2) / 2
        ...
        '''for angle in [start_angle, end_angle]:
            end_x = center_x + math.cos(angle) * self.view_ditan
            end_y = center_y + math.sin(angle) * self.view_ditan
        ...
        pygame.draw.polygon(self.maska_surface, (255, 255, 255, 255), points)
        return self.maska_surface
    ...
    def polica_saw_player(self, i):
        ...
        if dir_pol == 1:
            if player_y < polica_y and player_x == polica_x:
                return True
            else:
                return False
        elif dir_pol == 2:
            if player_x > polica_x and player_y == polica_y:
                return True
            else:
                return False
        elif dir_pol == 3:
    ...
    def draw_game_board(self): 
        ...
        self.tame_map_surface = pygame.Surface((WIDTH, HEIGHT))
        self.tame_map_surface.blit(self.map_surface, (0, 0))
        ...
        for y in range(self.start_y, self.start_y + rows * TILE_SIZE, TILE_SIZE):
            ...
            for x in range(self.start_x, self.start_x + colums * TILE_SIZE, TILE_SIZE):
                ...
                if s == "#":
                    self.map_surface.blit(self.floor_img, (x, y))
                    self.map_surface.blit(self.wall_img, (x, y))
                elif s == "-":
                    self.map_surface.blit(self.floor_img, (x, y))
                elif s == "X":
                    self.map_surface.blit(self.floor_img, (x, y))
                    self.map_surface.blit(self.x_img, (x, y))
                elif s == "%":
                    if self.kit_bool:
                        self.map_surface.blit(self.floor_img, (x, y))
                        self.map_surface.blit(self.kit_img, (x, y))
                    else:
                        self.map_surface.blit(self.floor_img, (x, y))
                if (i, j) in self.level_copy["player"]:
                    if self.dir == "up":
                        self.map_surface.blit(self.player_up_img, (x, y))
                    elif self.dir == "down":
                        self.map_surface.blit(self.player_down_img, (x, y))
                    elif self.dir == "left":
                        self.map_surface.blit(self.player_left_img, (x, y))
                    elif self.dir == "right":
                        self.map_surface.blit(self.player_right_img, (x, y))
                if (i, j) in self.level_copy["bomba"]:
                    self.map_surface.blit(self.bomba_img, (x, y))
                if (i, j) in self.level_copy["polica"]:
                    self.dir_pol, self.hp_pol = self.level_copy["dop_polica"][i1]
                    i1 += 1
                    if self.hp_pol > 0:
                        if self.dir_pol == 1:
                            self.map_surface.blit(self.polica_up_img, (x, y))
                        elif self.dir_pol == 2:
                            self.map_surface.blit(self.polica_right_img, (x, y))
                        elif self.dir_pol == 3:
                            self.map_surface.blit(self.polica_down_img, (x, y))
                        elif self.dir_pol == 4:
                            self.map_surface.blit(self.polica_left_img, (x, y))
                    else:
                        self.map_surface.blit(self.polica_death_img, (x, y))
                if (i, j) in self.level_copy["box"]:
                    self.map_surface.blit(self.box_img, (x, y))
        ...
        for bullet in self.bullets:
            if self.bullets[0]['dir'] == "up":
                ...
                self.map_surface.blit(self.bullet_up_img, (bullet_x, bullet_y))
            elif self.bullets[0]['dir'] == "down":
                bullet_x = self.start_x + bullet["x"] * TILE_SIZE
                bullet_y = self.start_y + bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_down_img, (bullet_x, bullet_y))
            elif self.bullets[0]['dir'] == "left":
                bullet_x = self.start_x + bullet["x"] * TILE_SIZE
                bullet_y = self.start_y + bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_left_img, (bullet_x, bullet_y))
            elif self.bullets[0]['dir'] == "right":
                bullet_x = self.start_x + bullet["x"] * TILE_SIZE
                bullet_y = self.start_y + bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_right_img, (bullet_x, bullet_y))
        ...
        for pol_bullet in self.pol_bullets:
            if self.pol_bullets[0]['dir'] == 'up':
                ...
                self.map_surface.blit(self.bullet_up_img, (pol_bullet_x, pol_bullet_y))
            elif self.pol_bullets[0]['dir'] == 'right':
                pol_bullet_x = self.start_x + pol_bullet["x"] * TILE_SIZE
                pol_bullet_y = self.start_y + pol_bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_right_img, (pol_bullet_x, pol_bullet_y))
            elif self.pol_bullets[0]['dir'] == 'down':
                pol_bullet_x = self.start_x + pol_bullet["x"] * TILE_SIZE
                pol_bullet_y = self.start_y + pol_bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_down_img, (pol_bullet_x, pol_bullet_y))
            elif self.pol_bullets[0]['dir'] == 'left':
                pol_bullet_x = self.start_x + pol_bullet["x"] * TILE_SIZE
                pol_bullet_y = self.start_y + pol_bullet["y"] * TILE_SIZE
                self.map_surface.blit(self.bullet_left_img, (pol_bullet_x, pol_bullet_y))


        rounded_cone = self.rounded_cone()
        self.tame_map_surface.blit(rounded_cone, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)
        self.screen.blit(self.tame_map_surface, (0, 0))
